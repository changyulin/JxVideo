...
namespace Jx.Services.Authentication.External
{
    ...
    public static partial class ExternalAuthorizerHelper
    {
        ...
        {
            var session = GetSession();
            session.Remove("Jx.externalauth.parameters");
        }

        public static void AddErrorsToDisplay(string error)
        {
            var session = GetSession();
            var errors = session["Jx.externalauth.errors"] as IList<string>;
            if (errors == null)
            {
                errors = new List<string>();
                session.Add("Jx.externalauth.errors", errors);
            }
            errors.Add(error);
        }
        ...
        public static IList<string> RetrieveErrorsToDisplay(bool removeOnRetrieval)
        {
            ...
            if (errors != null && removeOnRetrieval)
                session.Remove("Jx.externalauth.errors");
            return errors;
        }
    }
}